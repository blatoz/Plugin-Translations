...
public class AntiBotManager implements IAntiBotManager {
    ...
    public void checkModeDisable() {
        ...
        if(canDisable && timePasted) {
            ...
            ServerUtil.getInstance().getLogHelper().debug("[ANTIBOT MANAGER] A mód letiltásának jóváhagyása " + modeType);
        }

        if (timePasted) {
            attackDurationDetector.resetDuration();
            EventCaller.call(new AttackStateEvent(iAntiBotPlugin, AttackState.RUNNING, modeType));
            ServerUtil.getInstance().getLogHelper().debug("[ANTIBOT MANAGER] A mód letiltásának megtagadása " + modeType);
        }
    ...
    public boolean canDisable(ModeType modeType) {
        if (modeType.equals(ModeType.ANTIBOT) || modeType.equals(ModeType.SLOW)) {
            ServerUtil.getInstance().getLogHelper().debug("[ANTIBOT MANAGER] A kért üzemmód letiltása a következő esetében: " + modeType.name() + " eredmény " + (joinPerSecond.getSlowCount() <= ConfigManger.antiBotModeTrigger));
            return joinPerSecond.getSlowCount() <= ConfigManger.antiBotModeTrigger;
        }
        if (modeType.equals(ModeType.PING)) {
            ServerUtil.getInstance().getLogHelper().debug("[ANTIBOT MANAGER] A kért üzemmód letiltása a következő esetében: " + modeType.name() + " eredmény " + (pingPerSecond.getSlowCount() <= ConfigManger.pingModeTrigger));
            return pingPerSecond.getSlowCount() <= ConfigManger.pingModeTrigger;
        }
        if (modeType.equals(ModeType.PACKETS)) {
            ServerUtil.getInstance().getLogHelper().debug("[ANTIBOT MANAGER] A kért üzemmód letiltása a következő esetében: " + modeType.name() + " eredmény " + (packetPerSecond.getSlowCount() < ConfigManger.packetModeTrigger));
            return packetPerSecond.getSlowCount() < ConfigManger.packetModeTrigger;
        }
        ServerUtil.getInstance().getLogHelper().debug("[ANTIBOT MANAGER] A kért üzemmód letiltása a következő esetében: " + modeType);
        return false;
    }
